...
function Navbar({ setShowConnectWallet, user }) {
  ...
  const mobileLinkStyle =
    "flex items-center w-full gap-2 group p-4 hover:bg-[#F5F5F5]";
  const activeMobileLinkStyle =
    "flex items-center w-full gap-2 group p-4 bg-[#F5F5F5]";
      ...
      <AnimatePresence>
        <div className="container h-[80px] mx-auto px-6 lg:px-12 flex items-center relative">
          <div className="flex-1 flex space-x-4 items-center h-full">
            <span className="text-[#8cc4fc] bg-clip-text text-transparent lg:mr-8 text-2xl font-semibold bg-gradient-to-br from-[#000000] via-[#000000] to-[#F5FA80]">
              SWADEX
            </span>
          ...
          <div className="flex items-center space-x-4">
            <span className="hidden h-[40px] bg-[#D7E4E3] w-[74px] lg:flex items-center justify-center rounded-md font-medium">
              0 SAP
            </span>
            {user && (
              <Fragment>
                <h1 className="hidden lg:block text-xl font-medium">
                  4.8279 ETH
                </h1>
                <span className="hidden lg:block text-sm bg-[#F4F4F6] p-2 rounded-md">
                  0xBAD7...E18
                </span>
                <img src={accountOneAvatar} alt="" />
                <span className="hidden lg:block text-sm bg-[#F4F4F6] p-2 px-3 rounded-md cursor-pointer">
          ...
          <AnimatePresence>
            {showNotification && (
              <motion.div
                className="fixed bg-[#CDF9D0] rounded-md p-4 right-6 top-[90px] z-30"
                key="notification"
                initial={{ x: 100 }}
                animate={{ x: [100, 0] }}
              ...
              >
                ...
                <button className="font-semibold text-sm text-center w-full mt-2">
                  View on etherscan
                </button>
              </motion.div>
            )}
            ...
            >
              <span className="w-full flex justify-end pr-4">
                <ClearIcon
                  ...
                  className="text-right text-gray-600 cursor-pointer"
                />
              </span>
              <div className="w-full mt-4">
                <NavLink
                  to="/"
                  className={(navData) =>
                    navData.isActive ? activeMobileLinkStyle : mobileLinkStyle
                  }
                >
